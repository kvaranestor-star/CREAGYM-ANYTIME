// api/start-session.js — старт у КОНКРЕТНІЙ локації (за номером) + hold депозиту.
...
module.exports = async (req, res) => {
  ...
  try {
    const { location } = req.body || {};          // порядковий номер залу
    ...
    const row = await dbInsert('sessions', {
      ...
      tariff: t.id, rate: t.rate, hold_id: pay.invoiceId,
    });

    // TODO ESP32: POST {loc.esp32_url}/unlock — відкрити замок саме цієї локації
    return res.status(200).json({ ok: true, rate: t.rate, startedAt: row[0]?.started_at,
